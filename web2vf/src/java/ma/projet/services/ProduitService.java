...
public class ProduitService implements IDao<Produit> {
    ...
    public List<LigneCommandeProduit> findProduitByCommandeSansAffichage(Commande c) {
    SimpleDateFormat format = new SimpleDateFormat();
    System.out.println("Commande: " + c.getId() + "\t\tDate: " + format.format(c.getDate()));
    System.out.println("Liste des produits :");
    
    List<LigneCommandeProduit> commandeProduits = new ArrayList<>(); 
    Session session = null;
    Transaction tx = null;
    String query = "select l FROM LigneCommandeProduit l "
            + "join l.produit Produit "
    ...
    return commandeProduits; 
       ...
       public boolean update(int id, String ref , double prix,Categorie categorie) {
    Session session = null;
    Transaction tx = null;
    try {
        session = HibernateUtil.getSessionFactory().openSession();
  ...
  produit.setCategorie(categorie);
    ...
    public List<Produit> findProduitsPrixSuperieur() {
    List<Produit> produits = null;
    Session session = null;
    Transaction tx = null;
    try {
        ...
        query.setParameter("prixLimite", 100.0);
        produits = query.list();
        ...
        ex.printStackTrace(); // Ajoutez cette ligne pour afficher l'erreur.
    } finally {
        if (session != null) {
            session.close();
        }
